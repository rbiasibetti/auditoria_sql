...
CREATE PROCEDURE [DBO].[SP_CRIATRIGGERSAUDITORIA] 
AS BEGIN 
	SET NOCOUNT ON
	...
	DECLARE @TABLE_NAME SYSNAME SELECT @TABLE_NAME = MIN(TABLE_NAME) 
		...
		BEGIN 
			...
			SET @ISQL = 'CREATE TRIGGER DBO.TRG_AUD_' + @TABLE_NAME + '
				...
				----CASO OCORRAM ERROS NA CRIAÇÃO DA TRIGGER, O ERRO É RETORNADO
				...
				----CASO NÃO SEJA ENCONTRADA A TABELA COM O NOME DA BASE A SER AUDITADA JUNTO A BASE AUDITORIA, A PROCEDURE A CRIA AUTOMATICAMENTE
				...
				-----CASO NÃO EXISTA A VIEW DE CONSULTA DA AUDITORIA, A MESMA É CRIADA NA BASE
				IF OBJECT_ID('VW_AUDITORIA') IS NULL
					BEGIN
					EXEC ('CREATE VIEW [DBO].[VW_AUDITORIA] AS 
							SELECT  
							ID,DATA_OPERACAO,USUARIO_DB, 
							DBO.SEPARAR((REPLACE(REPLACE(USUARIO_PC,''.'',''.''),'';'',''#'')),1,''#'') AS USUARIO, 
							DBO.SEPARAR((REPLACE(REPLACE(USUARIO_PC,''.'',''.''),'';'',''#'')),2,''#'') AS APLICACAO, 
							DBO.SEPARAR((REPLACE(REPLACE(USUARIO_PC,''.'',''.''),'';'',''#'')),3,''#'') AS BASE, 
							DBO.SEPARAR((REPLACE(REPLACE(USUARIO_PC,''.'',''.''),'';'',''#'')),4,''#'') AS IP, 
							USUARIO_APP AS HOST,TABELA, 
							CASE OPERACAO 
								WHEN ''I'' THEN ''INCLUÍDO'' 
								WHEN ''D'' THEN ''EXCLUÍDO'' 
								WHEN ''U'' THEN ''ALTERADO'' 
							END OPERACAO, 
							OLD_DATA, 
							NEW_DATA 
							FROM AUDITORIA..'+@TABLE_BASE+'  WITH (NOLOCK)')
					END 
		END
GO
